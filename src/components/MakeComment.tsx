    ...
    return (
        <div>
            <form onSubmit={handleSubmit(onSubmit)} className="flex-col space-x-2">
                {error && error.message}
                <input type='text' placeholder="Остави коментар" {...register('body')} className=" bg-white placeholder-black rounded-md py-3 pl-2 pr-96 text-black"/>
                <button className=" bg-zinc-800 hover:bg-zinc-900 p-2 rounded-md px-4 py-3" type='submit'>Post</button>
            </form>
        </div>
    )
}
